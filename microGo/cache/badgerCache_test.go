package cache

import "testing"

func TestBadgerCache_Exists(t *testing.T) {
	err := testBadgerCache.Delete("foo")
	if err != nil {
		t.Error(err)
	}

	inCache, err := testBadgerCache.Exists("foo")
	if err != nil {
		t.Error(err)
	}

	if inCache {
		t.Error("foo found in cache, and it shouldn't be there")
	}

	_ = testBadgerCache.Set("foo", "bar")
	inCache, err = testBadgerCache.Exists("foo")
	if err != nil {
		t.Error(err)
	}

	if !inCache {
		t.Error("foo not found in cache")
	}

	err = testBadgerCache.Delete("foo")
	if err != nil {
		t.Error(err)
	}
}

func TestBadgerCache_Get(t *testing.T) {
	err := testBadgerCache.Set("foo", "bar")
	if err != nil {
		t.Error(err)
	}

	x, err := testBadgerCache.Get("foo")
	if err != nil {
		t.Error(err)
	}

	if x != "bar" {
		t.Error("did not get correct value from cache")
	}
}

func TestBadgerCache_Forget(t *testing.T) {
	err := testBadgerCache.Set("foo", "foo")
	if err != nil {
		t.Error(err)
	}

	err = testBadgerCache.Delete("foo")
	if err != nil {
		t.Error(err)
	}

	inCache, err := testBadgerCache.Exists("foo")
	if err != nil {
		t.Error(err)
	}

	if inCache {
		t.Error("foo found in cache, and it shouldn't be there")
	}

}

func TestBadgerCache_Clean(t *testing.T) {
	err := testBadgerCache.Set("alpha", "beta")
	if err != nil {
		t.Error(err)
	}

	err = testBadgerCache.Clean()
	if err != nil {
		t.Error(err)
	}

	inCache, err := testBadgerCache.Exists("alpha")
	if err != nil {
		t.Error(err)
	}

	if inCache {
		t.Error("alpha found in cache, and it shouldn't be there")
	}
}

func TestBadgerCache_DeleteIfMatch(t *testing.T) {
	err := testBadgerCache.Set("alpha", "beta")
	if err != nil {
		t.Error(err)
	}

	err = testBadgerCache.Set("alpha2", "beta2")
	if err != nil {
		t.Error(err)
	}

	err = testBadgerCache.Set("beta", "beta")
	if err != nil {
		t.Error(err)
	}

	err = testBadgerCache.DeleteIfMatch("a")
	if err != nil {
		t.Error(err)
	}

	inCache, err := testBadgerCache.Exists("alpha")
	if err != nil {
		t.Error(err)
	}

	if inCache {
		t.Error("alpha found in cache, and it shouldn't be there")
	}

	inCache, err = testBadgerCache.Exists("alpha2")
	if err != nil {
		t.Error(err)
	}

	if inCache {
		t.Error("alpha2 found in cache, and it shouldn't be there")
	}

	inCache, err = testBadgerCache.Exists("beta")
	if err != nil {
		t.Error(err)
	}

	if !inCache {
		t.Error("beta not found in cache, and it should be there")
	}
}
